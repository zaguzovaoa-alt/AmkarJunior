...
export const DirectorCRM: React.FC<DirectorCRMProps> = ({ setActiveTab }) => {
  ...
  // Lesson plan filter for Director (all / with_plan / without_plan)
  const [directorLessonPlanFilter, setDirectorLessonPlanFilter] = useState<"all" | "with_plan" | "without_plan">("all");

  const directorSessionsWithPlanCount = useMemo(() => {
    return (trainingSessions || []).filter((s) => Boolean(s.hasLessonPlan || s.lessonPlanText || s.lessonPlanPhotoUrl)).length;
  }, [trainingSessions]);

  const directorSessionsWithoutPlanCount = useMemo(() => {
    return (trainingSessions || []).filter((s) => !Boolean(s.hasLessonPlan || s.lessonPlanText || s.lessonPlanPhotoUrl)).length;
  }, [trainingSessions]);

  const filteredSessionsForDirector = useMemo(() => {
    return (trainingSessions || []).filter((s) => {
      const hasPlan = Boolean(s.hasLessonPlan || s.lessonPlanText || s.lessonPlanPhotoUrl);
      if (directorLessonPlanFilter === "with_plan") return hasPlan;
      if (directorLessonPlanFilter === "without_plan") return !hasPlan;
      return true;
    });
  }, [trainingSessions, directorLessonPlanFilter]);
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 text-gray-800 ">
      ...
      {activeSection === "metrics" ? (
        ...
        >
          ...
          {currentRole === "manager" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3.5 md:gap-4">
              {/* 1. Активные дети */}
              <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs text-left relative overflow-hidden flex flex-col justify-between min-h-[150px] hover:shadow-md transition">
                <div>
              ...
              <div
                ...
                className="bg-white p-4 rounded-2xl border border-amber-200/80 shadow-xs text-left relative overflow-hidden cursor-pointer hover:border-amber-300 hover:shadow-md transition group flex flex-col justify-between min-h-[150px]"
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1.5 flex-wrap">
              ...
              <div
                ...
                className="bg-white p-4 rounded-2xl border border-indigo-200/80 shadow-xs text-left relative overflow-hidden cursor-pointer hover:border-indigo-300 hover:shadow-md transition group flex flex-col justify-between min-h-[150px]"
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1.5 flex-wrap">
                  ...
                  <div className="text-xl sm:text-2xl font-black text-indigo-700 mt-1 tracking-tight">
                    {pendingTasksCount}
                  </div>
                </div>
                <MiniSparklineChart
              ...
              <div
                ...
                className="bg-white p-4 rounded-2xl border border-emerald-200/80 shadow-xs text-left relative overflow-hidden flex flex-col justify-between min-h-[150px] cursor-pointer hover:border-emerald-300 hover:shadow-md transition group"
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1.5 flex-wrap">
                  ...
                  <div className="text-xl sm:text-2xl font-black text-emerald-700 mt-1 tracking-tight">
                    {avgAttendanceRate}%
                  </div>
                </div>
                <MiniSparklineChart
              ...
              <div
                ...
                className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs text-left relative overflow-hidden cursor-pointer hover:border-slate-300 hover:shadow-md transition group flex flex-col justify-between min-h-[150px]"
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1.5 flex-wrap">
                    <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3.5 md:gap-4">
              ...
              {/* 2. Поступления сегодня (Clickable) */}
              <div
                onClick={() => setShowTodayPaymentsModal(true)}
                className="bg-white p-4 rounded-2xl border border-emerald-200/80 shadow-xs text-left relative overflow-hidden cursor-pointer hover:border-emerald-300 hover:shadow-md transition group flex flex-col justify-between min-h-[150px]"
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1.5 flex-wrap">
                    <span className="text-xs font-bold text-emerald-900 uppercase tracking-wide">
                    ...
                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 font-bold text-[10.5px] rounded-full shrink-0 whitespace-nowrap">
              ...
              {/* 6. Долги родителей (Clickable) */}
              <div
                onClick={() => setShowDebtorsModal(true)}
                className="bg-white p-4 rounded-2xl border border-rose-200/80 shadow-xs text-left relative overflow-hidden cursor-pointer hover:border-rose-300 hover:shadow-md transition group flex flex-col justify-between min-h-[150px]"
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1.5 flex-wrap">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                ...
                {/* Lesson Plan Quick Filter */}
                <div className="flex items-center justify-between gap-2 flex-wrap pb-1">
                  <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl text-xs font-bold">
                    <button
                      type="button"
                      onClick={() => setDirectorLessonPlanFilter("all")}
                      className={`px-3 py-1 rounded-lg transition ${
                        directorLessonPlanFilter === "all"
                          ? "bg-white text-slate-900 shadow-xs"
                          : "text-slate-600 hover:text-slate-900"
                      }`}
                    >
                      Все ({trainingSessions.length})
                    </button>
                    <button
                      type="button"
                      onClick={() => setDirectorLessonPlanFilter("with_plan")}
                      className={`px-3 py-1 rounded-lg transition flex items-center gap-1 ${
                        directorLessonPlanFilter === "with_plan"
                          ? "bg-white text-emerald-700 shadow-xs"
                          : "text-slate-600 hover:text-emerald-700"
                      }`}
                    >
                      <FileText className="w-3 h-3 text-emerald-600" />
                      С конспектом ({directorSessionsWithPlanCount})
                    </button>
                    <button
                      type="button"
                      onClick={() => setDirectorLessonPlanFilter("without_plan")}
                      className={`px-3 py-1 rounded-lg transition flex items-center gap-1 ${
                        directorLessonPlanFilter === "without_plan"
                          ? "bg-white text-rose-700 shadow-xs"
                          : "text-slate-600 hover:text-rose-700"
                      }`}
                    >
                      <AlertTriangle className="w-3 h-3 text-rose-600" />
                      Без конспекта ({directorSessionsWithoutPlanCount})
                    </button>
                  </div>
                  <span className="text-[11px] text-slate-400 font-medium">
                    Показано: {filteredSessionsForDirector.length} из {trainingSessions.length}
                  </span>
                </div>

                <div className="space-y-3">
                  {filteredSessionsForDirector.length === 0 ? (
                    ...
                    filteredSessionsForDirector.map((session) => (
                      <div
                        key={session.id}
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <div className="flex space-x-3 text-[10px] pt-1 flex-wrap items-center">
                ...
                <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
                  ...
                  <div
                    className={`p-4 rounded-2xl border ${
                      selectedSessionDetail.hasLessonPlan ||
                      selectedSessionDetail.lessonPlanText ||
                      selectedSessionDetail.lessonPlanPhotoUrl
                  ...
                  >
                    ...
                    {selectedSessionDetail.lessonPlanText && (
                      <div className="bg-white p-3.5 rounded-xl border border-emerald-200/70 text-xs text-slate-850 whitespace-pre-wrap leading-relaxed">
                        {selectedSessionDetail.lessonPlanText}
                      </div>
                    )}
                    ...
                    {selectedSessionDetail.lessonPlanPhotoUrl && (
                      <div className="mt-3">
                        ...
                        <div
                          className="w-48 h-36 rounded-xl overflow-hidden border border-emerald-200 shadow-xs relative cursor-pointer group"
                          onClick={() =>
                            setFullScreenPhoto(
                              selectedSessionDetail.lessonPlanPhotoUrl,
                            )
                        ...
                        >
                          <img
                            src={selectedSessionDetail.lessonPlanPhotoUrl}
                            alt="Конспект"
                            className="w-full h-full object-cover group-hover:scale-105 transition-transform"
                            referrerPolicy="no-referrer"
                          />
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition flex items-center justify-center">
                            <span className="opacity-0 group-hover:opacity-100 text-white font-bold bg-black/60 px-2.5 py-1 rounded-full text-[11px]">
                              Увеличить
                            </span>
                          </div>
                        </div>
                    ...
                    {!selectedSessionDetail.hasLessonPlan &&
                      ...
                      !selectedSessionDetail.lessonPlanPhotoUrl && (
                        <p className="text-xs text-rose-700 italic">
                          Тренер зафиксировал отметку «Нет конспекта» при заполнении отчета по тренировке.
                        </p>
                      )}
                  </div>
