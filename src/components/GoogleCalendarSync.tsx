...
export const GoogleCalendarSync: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'calendar' | 'sync'>('calendar');
  ...
  const handleManualSync = () => {
    ...
    setTimeout(() => {
      ...
      alert('Интеграция с Google Calendar: Синхронизация расписания выполнена успешно!\n6 событий тренировок экспортировано на ваш аккаунт zaguzovaoa@gmail.com.');
    }, 1200);
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 text-gray-800 min-h-screen">
      ...
      <div className="bg-white border-b border-gray-200 px-6 pt-6 pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0 z-30 shadow-xs">
        <div>
          <div className="text-xs font-black text-red-650 uppercase font-mono tracking-widest mb-1 select-none">Панель управления расписанием</div>
          <h1 className="text-2xl font-bold text-slate-950 font-sans tracking-tight">Календарь & Синхронизация</h1>
        </div>
        ...
        <div className="flex space-x-2 select-none self-start sm:self-auto">
          ...
          >
            ...
            {calendarSyncEnabled && <span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping"></span>}
          </button>
        </div>
      </div>
          ...
          <div className="p-6 max-w-4xl mx-auto space-y-6 text-left animate-in fade-in duration-100">
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-6 relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b pb-4">
                <div>
                  <h3 className="text-sm font-black uppercase text-slate-500 font-mono tracking-wider">Рабочее окружение</h3>
                  <p className="text-sm text-slate-900 font-bold mt-1">Интегрированный рабочий почтовый ящик: <span className="text-red-600 font-bold underline">zaguzovaoa@gmail.com</span></p>
                </div>

                <div className="flex items-center space-x-3.5 shrink-0">
                  <span className={`px-3 py-1 text-[10px] font-black tracking-wider uppercase rounded-full border ${
                    calendarSyncEnabled 
                      ? 'bg-emerald-50 border-emerald-100 text-emerald-800' 
                      : 'bg-amber-50 border-amber-100 text-amber-850'
                  }`}>
                    {calendarSyncEnabled ? '● Синхронизация АКТИВНА' : '● Синхронизация ОТКЛЮЧЕНА'}
                  </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs font-sans">
                <div className="p-4 bg-slate-50 border rounded-2xl space-y-3">
                  ...
                  <p className="text-gray-500 leading-normal">
                    При создании тренировочного дня, переносе корта или отмене матча тренером через интерактивный календарь, футбольная CRM автоматически производит PATCH-запросы к Google Calendar API, мгновенно отправляя обновления на телефоны родителей учеников.
                  </p>
                </div>
                ...
                <div className="p-4 bg-slate-50 border rounded-2xl space-y-3">
                  ...
                  <p className="text-gray-500 leading-normal mb-1">
                    Для перезаписи всех событий, исключения наложений и выравнивания времени тренировок в календарях родителей запустите полный экспорт данных за текущий месяц.
                  </p>
                  <button 
                  ...
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${testLogLoading ? 'animate-spin' : ''}`} />
                    <span>{testLogLoading ? 'Выполняем экспорт...' : 'Запустить экспорт в Google Calendar'}</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-4">
              <div className="flex border-b pb-3 items-center justify-between">
                ...
                <span className="text-[10px] bg-slate-100 text-emerald-700 font-extrabold px-2 py-0.5 rounded font-mono">ШЛЮЗ СВЯЗИ: 200 OK</span>
              </div>

              <div className="space-y-3 text-xs leading-normal font-sans">
                {calendarSyncLog.length === 0 ? (
                  <div className="py-6 text-center text-gray-450 italic">Логи транзакций пусты. Сделайте изменения в календаре или выполните экспорт.</div>
                ) : (
                  calendarSyncLog.map((log, id) => (
                    <div key={id} className="p-3 bg-slate-50 border-l-4 border-emerald-500 rounded-r-xl flex justify-between items-start">
                      <div className="space-y-0.5">
                        <div className="font-semibold text-slate-805">{log}</div>
                        <div className="text-[10px] text-gray-400 font-mono">Драйвер: Google API OAuth2</div>
                      </div>
                      <span className="text-[10.5px] font-mono font-bold text-gray-500">200 OK</span>
                    </div>
                  ))
                )}
              </div>
