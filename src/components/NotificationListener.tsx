...
export const NotificationListener: React.FC = () => {
  const { notifications, currentRole, userProfile, markNotificationRead, groups, clients } = useCRM();
  ...
  useEffect(() => {
    ...
    function initializeFCM() {
      if (!messaging) return;
      getToken(messaging, { vapidKey: process.env.VITE_FCM_VAPID_KEY || 'dummy_vapid_key' })
        .then((token) => {
           if (token) {
             setFcmToken(token);
           ...
           console.warn('FCM Token generation failed (expected in sandbox without VAPID keys):', err);
        });
  ...
  useEffect(() => {
    ...
    const relevantUnread = notifications.filter(notif => {
      ...
      // Role check
      if (notif.targetRole && notif.targetRole.length > 0 && !notif.targetRole.includes(currentRole)) {
        return false;
      }

      // If parent, check group membership
      if (currentRole === 'parent' && notif.targetGroupIds && notif.targetGroupIds.length > 0) {
        ...
        const myGroupIds = groups.filter(g => 
    ...
    const newlyUnread = relevantUnread.filter(n => !activeToasts.includes(n.id));
  ...
  }, [notifications, currentRole, browserPermission, activeToasts, clients, userProfile.phone, groups]);
        ...
        {toastsToRender.map(notif => (
          <motion.div
            ...
            className={`w-80 p-4 rounded-2xl shadow-2xl pointer-events-auto relative border ${
              notif.type === 'event' ? 'bg-fuchsia-50 border-fuchsia-200 text-fuchsia-900' :
              notif.type === 'system' ? 'bg-red-50 border-red-200 text-red-900' :
