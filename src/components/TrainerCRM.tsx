...
export const TrainerCRM: React.FC<TrainerCRMProps> = ({ activeTab, setActiveTab }) => {
  const { clients, tasks, groups, coaches, leads, finances, messages, trainingSessions, addChatMessage, updateChatMessage, deleteChatMessage, markAttendance, completeTrialAndMarkAttendance, ratePlayer, completeTask, updateClient, currentRole, userProfile } = useCRM();
  const { appUser } = useAuth();
  
  // Try to find coach based on logged in user's full name, email, or phone.
  const myCoach = coaches.find(c => 
  ...
  const visibleMessages = messages.filter(m => !m.visibleTo || m.visibleTo.includes(currentRole) || m.senderRole === currentRole);
  ...
  const submitAttendanceToCRM = () => {
    ...
    const clientRecords: { clientId: string; status: 'present' | 'absent_sick' | 'absent'; reason?: string }[] = [];
    ...
    markAttendance(selectedGroupForAttendance, formattedDate, clientRecords, uploadedAttendancePhoto || 'фотоотчет_тренировки_сп.jpg', sessionNotes);
          ...
          <div id="trainer-main-dashboard" className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="bg-white p-4 rounded-xl border border-gray-105 shadow-xs text-left">
                ...
                <div className="text-2xl font-black text-slate-900 mt-1">{myGroups.length}</div>
                <div className="text-[10px] text-gray-400 mt-0.5">{myGroups.reduce((acc, g) => acc + g.playersCount, 0)} игроков закреплено</div>
              </div>
              <div className="bg-white p-4 rounded-xl border border-gray-105 shadow-xs text-left">
                <span className="text-gray-400 font-bold uppercase text-[9px] tracking-wider font-mono">Тренировок в неделю</span>
                <div className="text-2xl font-black text-slate-900 mt-1">{myGroups.reduce((acc, g) => acc + (g.scheduleDays?.length || 0), 0)}</div>
                <div className="text-[10px] text-gray-400 mt-0.5">В соответствии с расписанием</div>
              </div>
              <div className="bg-white p-4 rounded-xl border border-gray-105 shadow-xs text-left">
                ...
                <div className="text-2xl font-black text-emerald-600 mt-1">
                  {(() => {
                    ...
                    const totalPlayers = mySessions.reduce((sum, s) => sum + s.presentCount + s.sickCount + s.absentCount, 0);
                    const present = mySessions.reduce((sum, s) => sum + s.presentCount, 0);
                    return totalPlayers > 0 ? `${Math.round((present / totalPlayers) * 100)}%` : '0%';
                  })()}
                </div>
                <div className="text-[10px] text-emerald-600 mt-0.5 font-bold">На основе {trainingSessions?.filter(s => s.coachId === myCoach.id || s.coachName.includes(myCoach.name)).length || 0} табелей</div>
              </div>
              <div className="bg-white p-4 rounded-xl border border-gray-105 shadow-xs text-left">
                <span className="text-gray-400 font-bold uppercase text-[9px] tracking-wider font-mono">Оценка игроков</span>
                <div className="text-2xl font-black text-amber-500 mt-1">
                  {(() => {
                     const localMyClients = myClients.filter(c => myGroups.some(g => g.name === c.groupName));
                ...
                <div className="text-[10px] text-gray-400 mt-0.5">Средний балл ваших групп</div>
              </div>
              <div className="bg-white p-4 rounded-xl border border-gray-105 shadow-xs col-span-2 lg:col-span-1 text-left">
                <span className="text-gray-400 font-bold uppercase text-[9px] tracking-wider font-mono">Задачи на день</span>
                ...
                <div className="text-[10px] text-orange-500 font-bold">{coachTasks.filter(t => t.status !== 'completed' && new Date(t.dueDate) < new Date()).length} просрочено</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
                  <div className="flex border-b pb-3 items-center justify-between">
                    <h3 className="font-extrabold text-slate-950 text-sm">Расписание на сегодня</h3>
                    <div className="flex items-center space-x-2 text-xs font-bold text-gray-500">
                      <Calendar className="w-4 h-4" />
                      <span>{new Date().toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', weekday: 'long' })}</span>
                    </div>
                  </div>
                        ...
                        return (
                          <div key={g.id} className="p-0 border-b last:border-0 border-gray-100 flex items-center justify-between py-3">
                            ...
                            <div className="flex-1 px-4 text-left">
                                ...
                                <div className="text-[10px] text-gray-400 mt-1">Тренировка</div>
                            </div>
                            <div className="flex items-center space-x-2">
                                <span className="text-[11px] font-bold text-slate-700">{g.playersCount} игроков</span>
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm text-left flex flex-col md:flex-row gap-6">
                    ...
                    <div className="w-full md:w-32 flex flex-col justify-center space-y-4">
                        ...
                        <div>
                            <div className="text-[10px] text-gray-400 font-medium">Всего тренировок</div>
                            <div className="text-xl font-bold text-slate-800 leading-none">6</div>
                        </div>
                        <div>
                            <div className="text-[10px] text-gray-400 font-medium">Всего посещений</div>
                            <div className="text-sm font-bold text-slate-800 leading-none">92 из 106</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4 text-left">
                  <div className="flex border-b pb-3 items-center justify-between">
                    <h3 className="font-extrabold text-slate-950 text-sm">Задачи</h3>
                  </div>
                  ...
                  <div className="space-y-3 text-xs">
                    {coachTasks.map((t, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex items-center justify-between py-2 border-b last:border-0 border-gray-50 group">
                            <div className="flex items-center space-x-3">
                                <button 
                                    ...
                                    className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${
                                        t.status === 'completed' ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-gray-300 hover:border-gray-400'
                                    }`}
                                >
                                    {t.status === 'completed' && <Check className="w-2.5 h-2.5" />}
                                </button>
                                <div className="space-y-0.5">
                                    <div className={`font-semibold ${t.status === 'completed' ? 'line-through text-gray-400' : 'text-slate-800'}`}>
                                        {t.title}
                                    </div>
                                    <div className="text-[10px] text-gray-400">Группа {idx % 2 === 0 ? '2014' : '2015'}</div>
                                </div>
                            </div>
                            <div className={`text-[10px] font-bold ${isOverdue ? 'text-rose-500' : 'text-gray-500'}`}>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
                  <div className="flex border-b pb-3 items-center justify-between">
                    <h3 className="font-extrabold text-slate-950 text-sm">Мои группы</h3>
                    <span onClick={() => setActiveTab('trainer_groups')} className="text-xs font-bold text-emerald-600 hover:underline cursor-pointer">Все группы</span>
                  </div>
                  ...
                  <div className="space-y-2">
                    {myGroups.map((grp, idx) => {
                      ...
                      return (
                      <div key={idx} className="py-3 border-b border-gray-50 last:border-0 flex justify-between items-center group cursor-pointer hover:bg-slate-50 transition px-2 rounded-lg -mx-2">
                        <div className="flex items-center space-x-3 text-left w-1/3">
                          <span className={`px-2 py-1 rounded font-bold text-[10px] ${badgeColor}`}>
                            {grp.year}
                          </span>
                        ...
                        <div className="flex space-x-4 flex-1 justify-end items-center text-xs">
                          <div className="text-left w-12 hidden sm:block">
                            <div className="font-bold text-slate-800 text-[11px]">{grp.playersCount}</div>
                            <div className="text-[9px] text-gray-400">Игроков</div>
                          </div>
                          <div className="text-left w-20 hidden lg:block">
                            <div className="font-bold text-slate-800 text-[11px]">{grp.attendanceRate}%</div>
                            <div className="text-[9px] text-gray-400">Посещаемость</div>
                          </div>
                          <div className="text-left w-12">
                            <div className="font-bold text-slate-800 text-[11px]">4.{8 - idx}</div>
                            <div className="text-[9px] text-gray-400">Оценка</div>
                          </div>
                          <button className="text-gray-300 hover:text-gray-600 font-bold px-1">⋮</button>
                        </div>
                      </div>
                    )})}
                  </div>
                  <div className="pt-2 flex justify-center border-t border-gray-50">
                      <span onClick={() => setActiveTab('trainer_groups')} className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700 cursor-pointer flex items-center mt-2">
                          Перейти ко всем группам <ChevronRight className="w-3.5 h-3.5 ml-1" />
                      </span>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
                  <div className="flex border-b pb-3 items-center justify-between">
                    <h3 className="font-extrabold text-slate-950 text-sm">Последние оценки игроков</h3>
                    <span onClick={() => setActiveTab('trainer_progress')} className="text-xs font-bold text-emerald-600 hover:underline cursor-pointer">Все игроки</span>
                  </div>
                        ...
                        <div className="flex items-center space-x-3 w-1/3">
                          {item.avatarUrl ? (
                            ...
                            <div className="w-8 h-8 rounded-full bg-slate-200 border border-slate-300 flex items-center justify-center text-slate-500 font-bold text-[10px] uppercase">
                              {item.childSurname?.[0]}{item.childName?.[0]}
                            </div>
                          )}
                          <div>
                            <div className="font-bold text-slate-800 text-xs">{item.childSurname} {item.childName}</div>
                            <div className="text-[9px] text-gray-400 font-medium">{item.groupName}</div>
                          </div>
                        </div>
                        
                        <div className="flex items-center space-x-1 w-10">
                            <span className="font-bold text-emerald-600 text-xs">{avg}</span>
                        </div>
                        
                        <div className="hidden sm:flex text-[9px] text-gray-500 w-16">
                            {id === 0 ? 'Дисциплина' : id === 1 ? 'Техника' : 'Тактика'}
                        </div>
                        
                        <div className="flex items-center space-x-1 w-8">
                            <span className="font-bold text-slate-800 text-[11px]">4.{8-id}</span>
                        </div>
                        
                        <div className="hidden lg:flex text-[9px] text-gray-400 truncate w-24">
                            {id === 0 ? 'Физ. подготовка' : id === 1 ? 'Обход фишек' : 'Игра в защите'}
                        </div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4 text-left">
                  <div className="flex border-b pb-3 items-center justify-between">
                    <h3 className="font-extrabold text-slate-950 text-sm">Последние файлы и материалы</h3>
                    <span onClick={() => setActiveTab('trainer_knowledge')} className="text-xs font-bold text-emerald-600 hover:underline cursor-pointer">Все материалы</span>
                  </div>
                  ...
                  <div className="space-y-4 w-full relative">
                    {[
                      ...
                      { name: 'Методика для тренеров', size: 'PDF • 3.4 МБ', date: 'Загружено 18.05.2025' }
                    ].map((file, idx) => (
                      <div key={idx} className="flex items-start space-x-3 w-4/5 pb-2 border-b last:border-0 border-gray-50">
                        <div className="w-8 h-8 rounded bg-rose-50 flex items-center justify-center shrink-0">
                          <span className="text-[8px] font-black text-rose-600">PDF</span>
                        </div>
                        <div className="space-y-0.5">
                          <div className="font-bold text-slate-800 text-xs truncate" title={file.name}>{file.name}</div>
                          <div className="text-[10px] text-gray-400 font-medium">{file.size} • {file.date}</div>
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {myGroups.map((g, idx) => {
                    ...
                    return (
                    <div key={idx} className={`p-5 bg-white border-2 rounded-2xl transition cursor-pointer flex flex-col justify-between ${colorClass}`} onClick={() => startAttendanceMarking(g.id)}>
                      <div className="mb-4">
                        <div className="font-black text-slate-900 text-lg">{g.name}</div>
                        <p className="text-[11px] text-gray-500 mt-1 font-medium">Обучается: <span className="font-black text-slate-700">{g.playersCount}</span> воспитанников</p>
                      </div>
                      <button 
                        className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl text-[11px] uppercase tracking-wider transition"
                      >
                        Заполнить табель
                      </button>
                    </div>
                  ...
                  {myGroups.length === 0 && (
                    <div className="col-span-1 sm:col-span-2 md:col-span-3 p-10 text-center bg-slate-50 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center">
                      ...
                      <h4 className="font-bold text-slate-900 mb-1">Нет закрепленных групп</h4>
                      <p className="text-xs text-gray-500">У вас пока нет групп для проведения тренировок.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="p-5 bg-white border border-gray-150 rounded-2xl space-y-4 shadow-sm text-left">
                  <div className="font-extrabold text-slate-900 text-xs flex items-center space-x-2">
                    ...
                    <span>Фиксация достоверности (Фотоотчет проведенной тренировки)</span>
                  </div>
                  <p className="text-[11px] text-gray-500 leading-relaxed">
                    Для автоматического подтверждения тренировки и отправки отчета директору прикрепите снимок построения группы или выберите фокус сегодняшнего занятия из шаблонов спортивной площадки:
                  </p>
                  
                  {/* Grid of professional training session templates */}
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
                    {[
                      {
                        name: 'Разминка и ОФП',
                        url: 'https://images.unsplash.com/photo-1517649763962-0c623066013b?w=400&auto=format&fit=crop&q=60'
                      },
                      {
                        name: 'Дриблинг и пас',
                      ...
                      },
                      {
                        ...
                        url: 'https://images.unsplash.com/photo-1544698310-74ea9d1c8258?w=400&auto=format&fit=crop&q=60'
                      },
                      {
                        name: 'Игровая практика',
                        url: 'https://images.unsplash.com/photo-1518063319789-7217e6706b04?w=400&auto=format&fit=crop&q=60'
                      }
                    ].map((tpl) => (
                      <button
                        key={tpl.name}
                        type="button"
                        onClick={() => setUploadedAttendancePhoto(tpl.url)}
                        className={`group relative h-20 rounded-xl overflow-hidden border-2 text-left transition-all ${
                          uploadedAttendancePhoto === tpl.url 
                            ? 'border-emerald-500 ring-2 ring-emerald-100 scale-[1.02]' 
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-black/60 flex items-end p-2">
                          <span className="text-[9px] text-white font-bold leading-tight">{tpl.name}</span>
                        </div>
                        {uploadedAttendancePhoto === tpl.url && (
                  ...
                  <div className="flex flex-col sm:flex-row gap-2.5 items-center pt-2">
                    <label className="flex-1 w-full bg-slate-50 border border-gray-200 rounded-lg shrink-0 cursor-pointer hover:bg-slate-100 transition relative">
                      <div className="flex items-center px-4 py-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-slate-700">Загрузить фото с устройства</span>
                          <span className="text-[10px] text-slate-400">JPG, PNG макс. 5МБ</span>
                        </div>
                      </div>
                      <input 
                        type="file"
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            compressImage(file, (base64) => setUploadedAttendancePhoto(base64));
                          }
                        }}
                      />
                    </label>
                    <button 
                      type="button" 
                      onClick={() => setUploadedAttendancePhoto('https://images.unsplash.com/photo-1517649763962-0c623066013b?w=400&auto=format&fit=crop&q=60')}
                  ...
                  {uploadedAttendancePhoto && uploadedAttendancePhoto.startsWith('data:image') && (
                    <div className="mt-3 relative w-full h-32 rounded-lg overflow-hidden border border-slate-200">
                      <img src={uploadedAttendancePhoto} alt="Preview" className="w-full h-full object-cover" />
                      <div className="absolute top-2 right-2 bg-emerald-500 text-white px-2 py-1 rounded text-[10px] font-bold">ФОТО ЗАГРУЖЕНО</div>
                    </div>
                  )}

                  <div className="text-xs font-mono text-gray-500 py-1 flex items-center justify-between">
                    <span>
                      {uploadedAttendancePhoto ? '✅ Фотоотчет подтвержден площадкой' : '⚠️ Фото не прикреплено (будет использована тренировочная заглушка)'}
                    </span>
                    {uploadedAttendancePhoto && (
                      <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full">подлинность подтверждена</span>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-2">
                  <div className="flex justify-between items-center">
                    <h4 className="font-extrabold text-slate-800 text-sm">Список воспитанников</h4>
                  </div>
                  <div className="flex flex-col space-y-2">
                    {(() => {
                      ...
                      const groupTrialLeads = (leads || []).filter(l => 
                        ...
                        l.trialGroupId && 
                        l.trialGroupId === groupObj?.id &&
                        l.trialDate === new Date().toISOString().split('T')[0]
                      ).map(l => ({
                        id: l.id,
                        childName: l.childName,
                        childSurname: l.childSurname,
                      ...
                      return allPlayers.map((p, i) => {
                        ...
                        return (
                          <div key={i} className="p-4 bg-white border border-gray-100 shadow-sm rounded-2xl flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:shadow-md transition">
                            <div className="flex items-center space-x-3 w-1/3">
                              <div className="w-10 h-10 rounded-full bg-slate-100 shrink-0 flex items-center justify-center font-bold text-slate-400">
                                {p.childSurname.charAt(0)}{p.childName.charAt(0)}
                              </div>
                              <div>
                                <div className="font-bold text-slate-850 text-sm">{p.childSurname} {p.childName}</div>
                                {isTrial && (
                ...
                <div className="space-y-2 pt-2">
                  <h4 className="font-extrabold text-slate-800 text-sm text-left mt-4 border-t pt-6">Заметки о тренировке (для директора)</h4>
                  <textarea 
                    value={sessionNotes}
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm text-left space-y-4">
            ...
            {!selectedPlayerForRating ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5">
                {myClients.filter(c => c.status === 'active').map((player, idx) => (
                  <div 
                    key={idx} 
                    onClick={() => {
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-[520px] overflow-hidden">
                    ...
                    <div className={`max-w-xs sm:max-w-md p-3 rounded-2xl group ${
                        ...
                        <div className="flex items-center space-x-2">
                          {isMe && (
                            <div className="hidden group-hover:flex items-center space-x-1 mr-2 bg-indigo-500/50 rounded px-1">
                              <button onClick={() => { setEditingMessageId(ms.id); setEditMessageText(ms.text); }} className="p-1 hover:text-white transition">
                                <Edit2 className="w-2.5 h-2.5" />
                              </button>
                              <button type="button" onClick={() => deleteChatMessage(ms.id)} className="p-1 hover:text-rose-200 transition">
                      ...
                      {editingMessageId === ms.id ? (
                        <div className="flex flex-col space-y-2 mt-2">
                          ...
                          <div className="flex justify-end space-x-2">
                            ...
                            <button onClick={() => { updateChatMessage(ms.id, editMessageText); setEditingMessageId(null); }} className="text-[10px] bg-white text-indigo-600 font-bold px-2 py-1 rounded">Сохр.</button>
                          </div>
                        </div>
                      ) : (
                      ...
                      {isMe && ms.visibleTo && (
                        <div className="mt-1 text-[8px] opacity-60 text-left">
                          Видно: {ms.visibleTo.map(r => r === 'director' ? 'Директор' : r === 'manager' ? 'Менеджер' : r === 'trainer' ? 'Тренер' : r === 'parent' ? 'Родители' : r).join(', ')}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-white border-t">
              <div className="px-3 py-2 flex items-center space-x-3 text-[10px] text-slate-500 border-b border-gray-50 overflow-x-auto">
                ...
                {(['manager', 'parent', 'director'] as const).map(role => (
                  <label key={role} className="flex items-center space-x-1 cursor-pointer whitespace-nowrap">
                    ...
                    />
                    <span>{role === 'manager' ? 'Менеджеры' : role === 'parent' ? 'Родители' : 'Директор'}</span>
                  </label>
                ))}
              </div>
              <form onSubmit={handleSendChat} className="p-3 flex items-center space-x-2">
                ...
                />
                <button type="submit" className="p-2.5 bg-indigo-600 hover:bg-indigo-700 rounded-xl text-white transition-all shadow-sm">
                    ...
                    <div key={idx} className="p-5 border rounded-2xl bg-white shadow-sm hover:shadow-md transition">
                      ...
                      <div className="space-y-3 pt-3 border-t text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold text-emerald-600">{grp.attendanceRate}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Расписание:</span>
                          <span className="font-bold text-slate-800">{grp.scheduleDays.join(', ')}</span>
                        </div>
                      </div>
                ...
                return (
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b pb-4 gap-3">
                      <div>
                        <h3 className="text-xl font-black text-slate-900">{grp.name}</h3>
                        <p className="text-xs text-gray-500 mt-1">Год рождения: {grp.year} &bull; Тренер: {myCoach.name}</p>
                      </div>
                      <div className="flex space-x-2">
                         <button onClick={() => setShowAddPlayerModal(true)} className="px-3 py-2 bg-emerald-50 border border-emerald-100 text-emerald-650 hover:bg-emerald-100 font-bold text-[11px] rounded-xl transition flex items-center space-x-1">
                      ...
                      {groupPlayers.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                          {groupPlayers.map(player => (
                            <div key={player.id} className="bg-white border rounded-2xl p-4 shadow-sm hover:shadow-md transition space-y-3 relative">
                               <div className="flex justify-between items-start">
                                  <div className="flex space-x-3 items-center">
                                    <div className="h-10 w-10 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-700 font-bold border border-emerald-200">
                                      {player.childSurname[0]}{player.childName[0]}
                                    </div>
                                    <div>
                                      <div className="font-bold text-slate-900 text-sm leading-tight">{player.childSurname} {player.childName}</div>
                                      <div className="text-[10px] text-gray-500 font-mono mt-0.5">{player.childAge} лет ({player.childBirthYear} г.р.)</div>
                                    </div>
                                  </div>
                                  <button onClick={async () => {
                                      if(window.confirm(`Вы уверены, что хотите исключить ${player.childName} из группы?`)) {
                                          await updateClient(player.id, { groupName: null });
                                      }
                                  }} className="p-1 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition" title="Исключить из группы">
                               ...
                               {editingPlayerId === player.id ? (
                                 <div className="bg-slate-50 rounded-xl p-3 border space-y-2 mt-2">
                                     <textarea 
                                       ...
                                       onChange={e => setEditPlayerNotes(e.target.value)}
                                     />
                                     <div className="flex justify-end space-x-2">
                                        <button onClick={() => setEditingPlayerId(null)} className="px-3 py-1.5 bg-white border text-gray-500 text-[10px] font-bold rounded-lg hover:bg-gray-50 transition">
                                            Отмена
                                        </button>
                                        <button onClick={async () => {
                                            await updateClient(player.id, { notes: editPlayerNotes });
                                  ...
                                  <div className="text-xs text-gray-600 bg-slate-50 hover:bg-slate-100/50 p-2.5 rounded-xl border flex justify-between items-start min-h-[60px] cursor-text">
                                     <span className={player.notes ? 'leading-relaxed' : 'text-gray-400 italic'}>{player.notes || 'Нет заметок по ученику. Нажмите, чтобы добавить.'}</span>
                                     <button onClick={() => {
                                         setEditingPlayerId(player.id);
                                         setEditPlayerNotes(player.notes || '');
                                     }} className="p-1 text-gray-400 hover:text-emerald-600 transition flex-shrink-0" title="Редактировать заметки">
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    ...
                    >
                      <div className="flex justify-between items-center p-4 border-b bg-slate-50">
                        <h3 className="font-extrabold text-slate-900 text-[13px] uppercase tracking-wider">Добавить ученика в {grp?.name}</h3>
                        <button onClick={() => setShowAddPlayerModal(false)} className="text-gray-400 hover:text-slate-900 transition">
                      ...
                      <div className="p-4 overflow-y-auto flex-1 space-y-2">
                        {unassignedPlayers.length === 0 ? (
                            <p className="text-center text-xs text-gray-500 py-4">Не найдено подходящих учеников</p>
                        ) : (
                          unassignedPlayers.map(p => (
                              <div key={p.id} className="flex items-center justify-between p-3 border rounded-xl hover:bg-slate-50 transition">
                                  <div className="flex items-center space-x-3">
                                      <div className="h-8 w-8 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-700 font-bold border border-emerald-200 text-xs">
                                          {p.childSurname[0]}{p.childName[0]}
                                      </div>
                                      <div>
                                          <div className="font-bold text-slate-800 text-xs">{p.childSurname} {p.childName}</div>
                                          <div className="text-[10px] text-gray-500 font-mono mt-0.5">{p.groupName ? `В группе: ${p.groupName}` : 'Без группы'}</div>
                                      </div>
                                  </div>
                                  <button onClick={async () => {
                                      await updateClient(p.id, { groupName: grp?.name });
        ...
        {activeTab === 'trainer_knowledge' && (
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm text-left space-y-6">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              <h3 className="text-lg font-bold text-slate-900">Учебно-методические материалы</h3>
              <button 
                onClick={() => alert('Открывается окно загрузки нового файла...')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                { name: 'Правила проведения матчей ПФЛ', size: 'DOCX • 54 КБ', date: '10 мая 2026' },
                { name: 'Презентация: Психология победителя', size: 'PPTX • 8.1 МБ', date: '01 мая 2026' }
              ].map((file, idx) => (
                <div key={idx} className="p-4 bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition">
                  <div className="w-12 h-12 bg-rose-50 rounded-xl flex items-center justify-center mb-4 border border-rose-100">
                    <span className="text-xs font-black text-rose-600 uppercase">{file.name.split('.').pop()?.substring(0,3) || file.size.split(' ')[0]}</span>
                  </div>
                  <h4 className="font-bold text-slate-900 text-sm mb-1 line-clamp-2" title={file.name}>{file.name}</h4>
                  <p className="text-xs text-gray-500 font-medium mb-4">{file.size} • {file.date}</p>
                  
                  <button 
                    onClick={() => alert(`Начата симуляция скачивания файла: ${file.name}`)}
