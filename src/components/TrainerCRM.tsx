...
export const TrainerCRM: React.FC<TrainerCRMProps> = ({
  ...
  // IMPORTANT: Sessions are only flagged if they were NOT filled on time ("вовремя не заполнили").
  // Today's sessions before training time ends are NOT flagged on the start of the day.
  const unreportedCoachSessions = useMemo(() => {
    ...
    for (let i = 0; i <= checkDays; i++) {
      ...
      myGroups.forEach((g) => {
        ...
        const slot = (g.scheduleDays || []).find((s) => s.startsWith(dayAbbr));
        if (!slot) return;

        // Check if report exists in trainingSessions using robust match
        ...
        // On today, if session has not finished yet, do NOT flag it!
        const isOverdue = isSessionOverdue(dateStr, slot);
        if (!isOverdue) return;

        const cleanSlot = slot || `${dayAbbr} 18:00`;
  ...
  const startAttendanceMarking = (groupId: string, targetDate?: string) => {
    ...
    if (targetDate) {
      setAttendanceDate(normalizeDateToYMD(targetDate));
    } else {
      setAttendanceDate(getLocalTodayYMD());
    }
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 text-gray-800 ">
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4 md:space-y-6">
              ...
              <div className="p-4 bg-gradient-to-r from-amber-500/10 via-amber-50 to-rose-50 border border-amber-300 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-3 text-left shadow-xs">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {unreportedCoachSessions.slice(0, 5).map((item) => (
                        <button
                          key={item.id}
                          type="button"
                          onClick={() => startAttendanceMarking(item.groupId, item.dateStr)}
                          className="text-[10px] font-semibold bg-white hover:bg-amber-100/90 active:bg-amber-200 border border-amber-300 text-amber-950 px-2.5 py-1 rounded-lg transition text-left cursor-pointer flex items-center gap-1.5 shadow-2xs"
                          title="Нажмите, чтобы заполнить табель за это число"
                        >
                          <span className="font-bold">{formatGroupNameDisplay(item.groupName)}</span>
                          <span className="text-amber-800 font-mono text-[9px]">({item.dateStr})</span>
                        </button>
                      ))}
                      {unreportedCoachSessions.length > 5 && (
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="text-[10px] font-bold text-amber-900 bg-amber-200/70 hover:bg-amber-300/80 px-2 py-1 rounded-lg transition self-center"
                        >
                          + ещё {unreportedCoachSessions.length - 5}
                        </button>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0 self-end md:self-center">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (unreportedCoachSessions[0]) {
                        startAttendanceMarking(unreportedCoachSessions[0].groupId, unreportedCoachSessions[0].dateStr);
                      } else {
                        setActiveTab("trainer_attendance");
                      }
                    }}
                    className="px-3.5 py-2 bg-white hover:bg-amber-100 text-amber-950 border border-amber-300 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-2xs"
                  >
                  ...
                  <div className="space-y-3">
                    {todaysGroups.length > 0 ? (
                      todaysGroups.map((g) => {
                        ...
                        const todayIsoStr = getLocalTodayYMD();

                        const isReportedToday = trainingSessions.some((ts) => {
                          return (
                            isGroupMatch(ts.groupId, ts.groupName, g.id, g.name) &&
                            isDateMatch(ts.date, ts.dateString, todayIsoStr)
                          );
                        });

                        // Stale or duplicate cancellation ignored if report is present
                        const todayCancellation = !isReportedToday && cancelledSessions.find((cs) => {
                          return (
                            isGroupMatch(cs.groupId, cs.groupName, g.id, g.name) &&
                            isDateMatch(cs.date, undefined, todayIsoStr)
                          );
                        });

                        const isSessionLate = isSessionOverdue(todayIsoStr, g.todayTime);
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center space-x-1.5 shrink-0">
                              {isReportedToday ? (
                                <span className="px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-800 font-bold text-[10px] flex items-center gap-1 border border-emerald-200 shadow-2xs">
                                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                                  <span>Табель сдан</span>
                                </span>
                              ) : todayCancellation ? (
                                <span className="px-2.5 py-1 rounded-lg bg-rose-100 text-rose-800 font-bold text-[10px] flex items-center gap-1 border border-rose-200 shadow-2xs">
                                  <Ban className="w-3.5 h-3.5 text-rose-600" />
                                  <span>Отменена ({todayCancellation.reason})</span>
                                </span>
                              ) : isSessionLate ? (
                                <>
                                  <span className="px-2 py-0.5 rounded-lg bg-amber-100 text-amber-900 font-bold text-[10px] flex items-center gap-1 border border-amber-300">
                                    <AlertTriangle className="w-3 h-3 text-amber-600" />
                                    <span>Не сдан вовремя</span>
                                  </span>
                                  <button
                                    type="button"
                                    onClick={() => startAttendanceMarking(g.id, todayIsoStr)}
                                    className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-lg text-[10px] uppercase tracking-wider transition shadow-xs"
                                  >
                                    Табель
                                  </button>
                                  <button
        ...
        <TrainerCancelModal
          ...
          coachId={myCoach.id}
          onFillAttendance={(groupId, dateStr) => {
            setShowTrainerCancelModal(false);
            startAttendanceMarking(groupId, dateStr);
          }}
          onSuccess={(count) => {
