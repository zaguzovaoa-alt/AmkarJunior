...
export const PaymentModal = ({ isOpen, onClose, clientId }: { isOpen: boolean, onClose: () => void, clientId: string }) => {
  const { clients, crmConfig } = useCRM();
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState(false);
  const [selectedTariff, setSelectedTariff] = useState<'12_sessions' | '8_sessions' | '4_sessions' | '1_session'>('12_sessions');
  const [amount, setAmount] = useState(0);

  useEffect(() => {
    if (!crmConfig || !crmConfig.tariffs) return;
    switch(selectedTariff) {
      case '12_sessions': setAmount(crmConfig.tariffs['12_sessions'] || 0); break;
      case '8_sessions': setAmount(crmConfig.tariffs['8_sessions'] || 0); break;
      case '4_sessions': setAmount(crmConfig.tariffs['4_sessions'] || 0); break;
      case '1_session': setAmount(crmConfig.tariffs['1_session'] || 0); break;
    }
  }, [selectedTariff, crmConfig]);

  if (!isOpen) return null;
  if (!crmConfig || !crmConfig.tariffs) return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm"><Loader2 className="w-8 h-8 animate-spin text-white" /></div>;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={!loading && !success ? onClose : undefined}></div>
      <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl relative z-10 overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="bg-slate-50 border-b border-slate-100 p-5 flex justify-between items-center relative">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-extrabold text-slate-800 text-lg">Оплата YooKassa</h3>
          </div>
          {(!loading && !success) && (
        ...
        <div className="p-6">
          {success ? (
            <div className="text-center space-y-4 py-8 animate-in zoom-in duration-500">
              ...
              <div>
                <h4 className="text-xl font-black text-slate-800">Оплата прошла успешно!</h4>
                <p className="text-sm text-slate-500 mt-2">Ваш абонемент продлен. Ссылка на чек отправлена на вашу почту.</p>
              </div>
              <button 
                onClick={() => {
                  setSuccess(false);
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">Выберите тариф</label>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <div className="text-[10px] text-slate-500 font-bold uppercase mb-1">12 занятий</div>
                    <div className="font-black text-slate-800 text-lg">{crmConfig?.tariffs?.['12_sessions'] || 0} ₽</div>
                  </div>
                  <div 
                  ...
                  >
                    <div className="text-[10px] text-slate-500 font-bold uppercase mb-1">8 занятий</div>
                    <div className="font-black text-slate-800 text-lg">{crmConfig?.tariffs?.['8_sessions'] || 0} ₽</div>
                  </div>
                  <div 
                  ...
                  >
                    <div className="text-[10px] text-slate-500 font-bold uppercase mb-1">4 занятия</div>
                    <div className="font-black text-slate-800 text-lg">{crmConfig?.tariffs?.['4_sessions'] || 0} ₽</div>
                  </div>
                  <div 
                  ...
                  >
                    <div className="text-[10px] text-slate-500 font-bold uppercase mb-1">Разовое</div>
                    <div className="font-black text-slate-800 text-lg">{crmConfig?.tariffs?.['1_session'] || 0} ₽</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-center space-x-3">
                 ...
                 <div className="text-[10px] text-blue-700 leading-tight">
                   Платеж защищен шифрованием. Мы не храним данные ваших карт. Операция проходит через сервис ЮKassa.
                 </div>
              </div>
