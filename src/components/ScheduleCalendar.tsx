...
export const ScheduleCalendar: React.FC<{ filteredGroupId?: string; filteredCoachId?: string; }> = ({ filteredGroupId, filteredCoachId }) => {
  ...
  const [showEventDetailsModal, setShowEventDetailsModal] = useState<CustomEvent | null>(null);

  // New deletion & clean up controller states
  const [selectedEventIds, setSelectedEventIds] = useState<string[]>([]);
  const [isDeleteModeActive, setIsDeleteModeActive] = useState(false);
  const [bypassDeleteConfirmation, setBypassDeleteConfirmation] = useState(false);
        ...
        parsedSlots.forEach(slot => {
          ...
          for (let d = 1; d <= daysInMonth; d++) {
            ...
            if (testDate.getDay() === targetDayOfWeek) {
              ...
              const coachName = activeCoach ? activeCoach.name : (g.coachName || 'Не указан');
  ...
  const allEvents = useMemo(() => {
    ...
    const taskEvents: CustomEvent[] = (tasks || [])
      ...
      .map(t => {
        ...
        const isRisk = t.title.toLowerCase().includes('риск') || t.title.toLowerCase().includes('risk');
    ...
    const combined = [...customEvents, ...taskEvents, ...repeatingScheduleEvents];
  ...
  const handleCreateEvent = (e: React.FormEvent) => {
    ...
    const generatedTitle = newEventType === 'regular' 
    ...
    if (newEventType === 'regular' && gObj) {
      ...
      const locPrefix = newEventLocation && newEventLocation !== 'Манеж Спартак' ? `${newEventLocation}: ` : '';
    ...
    if (id.startsWith('rep_')) {
      ...
      if (gObj) {
        ...
        const updatedSlots = gObj.scheduleDays.filter(s => {
          ...
          const hasMatch = parsed.some(slot => slot.day === dayAbbr && slot.time.replace(/[^0-9a-zA-Z]/g, '') === targetTime);
          return !hasMatch;
        });
  ...
  const handleBatchDelete = (ids: string[]) => {
    ...
    if (customIds.length > 0) {
      setCustomEvents(prev => prev.filter(ev => !customIds.includes(ev.id)));
    }
  ...
  const renderTypeBadge = (type: CustomEvent['type']) => {
    switch(type) {
      ...
      case 'masterclass':
        return <span className="px-2 py-0.5 bg-amber-100 text-amber-800 font-extrabold text-[9px] uppercase rounded border border-amber-200">Мастер-Класс</span>;
      case 'meeting':
        return <span className="px-2 py-0.5 bg-indigo-105 bg-indigo-100 text-indigo-800 font-extrabold text-[9px] uppercase rounded border border-indigo-200">Собрание</span>;
      case 'regular':
      default:
        return <span className="px-2 py-0.5 bg-emerald-50 text-emerald-800 font-extrabold text-[9px] uppercase rounded border border-emerald-100">Занятие по расписанию</span>;
    }
  };
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 text-gray-800 min-h-screen">
      ...
      <div className="p-6 bg-white border-b border-gray-150 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-950 font-sans">Интерактивное расписание академии</h1>
          <p className="text-gray-500 text-sm">Удобное составление, перенос и отмена тренировок футбольных групп с синхронной автовыгрузкой в Google-календари.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <span>{isDeleteModeActive ? 'Выйти из удаления' : 'Пакетное удаление'}</span>
          </button>
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 text-left">
          ...
          <div className="p-4 bg-red-50 border border-red-200 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-in slide-in-from-top-3 duration-150 shadow-sm relative overflow-hidden select-none">
            ...
            <div className="space-y-1 z-10">
              <div className="flex items-center space-x-2">
                ...
                <h4 className="font-extrabold text-xs text-red-950 uppercase tracking-tight font-mono">Режим быстрого и пакетного удаления</h4>
              </div>
              <p className="text-[11px] text-red-800 leading-normal max-w-2xl">
                Отмечайте записи флажками на страницах календаря, либо удаляйте их мгновенно кнопкой <Trash className="w-3.5 h-3.5 inline text-red-600 mx-0.5" />. Выбрано занятий для стирания: <strong className="text-red-950 font-mono text-xs bg-red-100 px-2 py-0.5 rounded-md border border-red-200">{selectedEventIds.length}</strong>
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto z-10">
              <label className="flex items-center space-x-2 bg-white px-3 py-2 rounded-xl border border-red-200 text-xs font-semibold text-red-900 cursor-pointer hover:bg-red-50/50 transition self-stretch md:self-auto select-none">
                <input
                  ...
                  onChange={(e) => setBypassDeleteConfirmation(e.target.checked)}
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative overflow-hidden">
          ...
          <div className="flex items-start space-x-3.5 z-10">
            <div className={`p-3 rounded-2xl shrink-0 ${calendarSyncEnabled ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-orange-50 text-orange-600 border border-orange-100'}`}>
              <RefreshCw className={`w-5.5 h-5.5 ${syncLoading ? 'animate-spin' : ''}`} />
            </div>

            <div>
              <div className="flex items-center space-x-2">
                <h3 className="font-extrabold text-slate-900 text-sm">Статус шлюза Google Calendar API</h3>
                <span className={`w-2 h-2 rounded-full ${calendarSyncEnabled ? 'bg-emerald-500 animate-pulse' : 'bg-orange-500'}`}></span>
              </div>
          ...
          <div className="flex items-center gap-3 z-10 shrink-0 w-full md:w-auto">
            ...
            >
              {calendarSyncEnabled ? 'Отключить интеграцию' : 'Активировать Google Sync'}
            </button>
                ...
                return (
                  <div
                    ...
                    className={`min-h-[110px] p-2 flex flex-col justify-between group relative transition-colors ${
                      isCellActive ? 'bg-white hover:bg-slate-50/70' : 'bg-slate-50/40 text-gray-300'
                  ...
                  >
                    ...
                    <div className="flex-1 space-y-1 overflow-y-auto max-h-[75px] scrollbar-none pb-0.5">
                      {cellEvents.map((ev) => {
                        ...
                        let bgTypeStyle = 'bg-emerald-50 text-emerald-800 border-emerald-100';
                        ...
                        return (
                          ...
                          >
                            <div className="flex flex-col min-w-0 flex-1 py-0.5 text-left leading-normal">
                              <div className="flex items-center space-x-1 min-w-0">
                                ...
                                <span className="truncate font-sans font-bold text-[10px] text-slate-900">
                                  {ev.title.startsWith('Регулярная тренировка: ') 
                                    ? ev.title.replace('Регулярная тренировка: ', '') 
                                    : ev.title}
                                </span>
                              </div>
                              {ev.coachName && (
                            ...
                            <div className="flex flex-col items-end justify-between shrink-0 ml-1.5 space-y-0.5 self-start pt-0.5">
                              <span className="text-[8px] font-black font-mono tracking-tighter bg-black/5 px-1 py-0.5 rounded text-slate-700 shrink-0">{ev.time}</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <span className={`text-[10px] font-extrabold uppercase font-mono tracking-wider ${w.isToday ? 'text-red-600' : 'text-slate-400'}`}>
                    ...
                    <div className="flex-grow space-y-2">
                      {dayEvents.length === 0 ? (
                        <div className="text-[10px] italic text-slate-350 text-center py-10">Нет записей</div>
                      ) : (
                        dayEvents.map((ev) => {
                            ...
                            >
                              <div className="flex items-center justify-between font-mono font-bold text-[9px] text-slate-500 mb-1">
                                ...
                                <div className="flex items-center space-x-1">
                                  <span className="uppercase text-[8px]">{ev.type}</span>
                                  <button
                                    type="button"
                                    onClick={(e) => {
                                      e.stopPropagation();
                              ...
                              <div className="font-extrabold line-clamp-2 leading-snug">{ev.title}</div>
                              <div className="text-[9px] text-gray-500 font-medium mt-1 truncate">Группа: {ev.groupName}</div>
        ...
        {view === 'list' && (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm text-left space-y-4">
            <h3 className="font-extrabold text-slate-950 text-sm">Предстоящие тренировки и события в хронологическом порядке</h3>
            
            {filteredEvents.length === 0 ? (
              <div className="text-center py-12 text-gray-450 italic text-xs">Нет тренировок с выбранными параметрами в календаре.</div>
            ) : (
                      ...
                      >
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="text-left shrink-0">
                            <span className="text-[10px] font-bold text-gray-400 block font-mono uppercase tracking-wider">ДАТА</span>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-neutral-900/40 backdrop-blur-xs flex items-center justify-center p-4 sm:p-6 z-50 text-left select-none">
          <div className="bg-white rounded-2xl max-w-md w-full border border-slate-100 shadow-2xl p-5 sm:p-6 relative max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
            ...
            <p className="text-xs text-gray-450 mb-4">Создайте постоянную тренировку или разовое спортивное событие в общем календаре.</p>

            <form onSubmit={handleCreateEvent} className="space-y-4">
              
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase font-mono block">Категория</label>
                  <select
                    value={newEventType}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase font-mono block">Футбольная группа</label>
                  <select
              ...
              {newEventType !== 'regular' && (
                <div className="space-y-1 animate-in fade-in duration-100">
                  <label className="text-[10px] font-bold text-slate-500 uppercase font-mono block">Название события</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase font-mono block">Дата события</label>
                  <input
                    type="date"
                    required
                    value={newEventDate}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase font-mono block">Время начала</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase font-mono block">Тренер-куратор</label>
                  <select
              ...
              {newEventType === 'regular' && (
                <div className="p-3 bg-red-50/50 border border-red-100 rounded-xl space-y-1">
                  ...
                  <p className="text-[10px] text-slate-500 leading-normal">
                    Поскольку выбрана категория регулярного занятия, этот день недели автоматически добавится в повторяемый график группы в CRM-системе.
                  </p>
                </div>
              )}
              ...
              {calendarSyncEnabled && (
                <div className="p-2.5 bg-emerald-50 text-emerald-850 rounded-xl border border-emerald-100 text-[10px] flex items-center space-x-2">
                  ...
                  <span>Google-выгрузка включена. Событие моментально появится в календаре родителей и тренеров.</span>
                </div>
              )}
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full border border-slate-100 shadow-2xl p-5 sm:p-6 relative max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-4">
              <div>
                ...
                <h3 className="text-base font-black text-slate-900 leading-snug">{showEventDetailsModal.title}</h3>
                <span className="text-[11px] font-bold text-slate-450 block font-mono mt-0.5">{showEventDetailsModal.groupName}</span>
              </div>
                ...
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <span className="font-bold block text-slate-800 font-mono">
                      {showEventDetailsModal.date.split('-')[2]}.{showEventDetailsModal.date.split('-')[1]}.{showEventDetailsModal.date.split('-')[0]} в {showEventDetailsModal.time}
                    </span>
                    <span className="text-[10px] text-gray-405 block font-medium">Регулярность: {showEventDetailsModal.type === 'regular' ? 'Каждую неделю' : 'Разовое'}</span>
                  </div>
                </div>
              ...
              {calendarSyncEnabled && (
                <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl flex items-center gap-2 text-[10px] text-slate-500">
                  ...
                  <span>Статус синхронизации: <strong>Успешно экспортировано в Google</strong></span>
                </div>
              )}
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full border border-slate-100 shadow-2xl p-6 relative animate-in zoom-in-95 duration-150">
            <div className="space-y-4">
              <div className="flex items-center space-x-3 text-red-600">
                ...
                <h3 className="text-sm font-black uppercase tracking-tight font-sans">Подтверждение удаления</h3>
              </div>
